<template>
  <div class="sample-detail-view fade-in">
    <!-- Header -->
    <div class="view-header" v-if="sample">
      <div class="header-left">
        <RouterLink to="/samples" class="back-btn">← Quay lại</RouterLink>
        <div class="title-wrap">
          <h2>{{ sample.name }}</h2>
          <code class="code-tag">{{ sample.code }}</code>
          <span class="badge" :class="sample.type === 'DEFECT' ? 'badge-critical' : 'badge-success'" style="margin-left: 8px">
            {{ sample.type === 'DEFECT' ? 'Lỗi' : 'Thành Phẩm' }}
          </span>
        </div>
      </div>
      <div class="header-right">
        <button class="btn btn-ghost icon-btn" @click="openSettings" title="Cài đặt mẫu vật">
    ...
    <!-- Info Card -->
    <div class="card info-card" v-if="sample">
      <div class="info-row">
        <span class="info-label">Trạng thái:</span>
        <span class="badge" :class="sample.isActive ? 'badge-active' : 'badge-archived'">
          {{ sample.isActive ? 'Hoạt động' : 'Tắt' }}
        </span>
      </div>
      <div class="info-row">
        <span class="info-label">Sản phẩm đích:</span>
        <span class="info-value">{{ sample.targetProductId ? (sample.targetProductId.code + ' - ' + sample.targetProductId.name) : 'Tất cả' }}</span>
      </div>
      <div class="info-row">
        <span class="info-label">Mô tả:</span>
        <span class="info-value">{{ sample.description || 'Chưa có mô tả' }}</span>
      </div>
    </div>

    <!-- Samples Section -->
    <div class="samples-section" v-if="sample">
      <div class="section-header">
        <h3>Ảnh Mẫu ({{ sampleImages.length }})</h3>
      ...
      <div v-else-if="sampleImages.length === 0" class="empty-view">
        <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" opacity="0.3">
          <rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>
        </svg>
        <p>Chưa có ảnh mẫu nào.</p>
        <button class="btn btn-ghost btn-sm" @click="triggerUpload">Bắt đầu tải lên</button>
      </div>

      <div v-else class="image-grid">
        <div v-for="(img, index) in sampleImages" :key="index" class="image-card">
          <div class="image-wrap">
            <img :src="`/${(img.filePath || img).replace(/\\/g,'/')}`" class="preview-img" @error="onImgError" />
            <div class="image-overlay">
              <button class="btn btn-danger btn-icon delete-btn" @click="deleteSampleImg(img.filePath || img)" title="Xoá ảnh">
    ...
    <div v-if="!sample && !loading" class="empty-view">
      <h3>Không tìm thấy mẫu vật</h3>
      <RouterLink to="/samples" class="btn btn-primary">Quay lại danh sách</RouterLink>
    </div>
      ...
      <div v-if="showEdit" class="modal-backdrop" @click.self="closeEdit">
        <div class="modal-box fade-in" style="max-width:480px">
          <div class="modal-header">
            <h3>Cài Đặt Mẫu Vật</h3>
            <button class="close-btn" @click="closeEdit">✕</button>
          </div>
          <div class="modal-body" style="flex-direction:column; gap:14px; padding: 20px 24px;">
            <div class="field">
              <label>Mã mẫu vật (Không thể sửa)</label>
              <input v-model="editForm.code" class="input" disabled />
            </div>
            <div class="field">
              <label>Phân loại (Không thể sửa)</label>
              <input :value="editForm.type === 'DEFECT' ? 'Lỗi Ngoại Quan' : 'Thành Phẩm'" class="input" disabled />
            </div>
            <div class="field">
            ...
            <div class="field" v-if="editForm.type === 'DEFECT'">
              <label>Sản Phẩm Đích <span style="color:#ef4444">*</span></label>
              <select v-model="editForm.targetProductId" class="input">
            ...
            <div class="field" style="margin-top: 10px;">
              <button class="btn btn-danger" @click="removeSample" style="width:100%">Xoá Mẫu Vật Này</button>
            </div>
          </div>
...
const sampleCode = route.params.code as string

const sample = ref<any>(null)
const allSamples = ref<any[]>([])
const productSamples = computed(() => allSamples.value.filter(s => s.type === 'PRODUCT'))
...
const editForm = reactive({ code: '', name: '', description: '', type: 'DEFECT', targetProductId: '' })
...
async function fetchSample() {
  ...
  try {
    ...
    allSamples.value = list
    sample.value = list.find((d: any) => d.code === sampleCode)
    if (sample.value) {
      await fetchSamples()
    }
  } catch (e) {
...
async function fetchSamples() {
  ...
  try {
    const data: any = await http.get(`/samples/${sampleCode}/images`)
    sampleImages.value = data
  } catch (e) {
...
async function handleUpload(event: Event) {
  ...
  return new Promise<void>((resolve) => {
    const next = async () => {
      ...
      try {
        await http.post(`/samples/${sampleCode}/images`, formData, {
...
async function deleteSampleImg(samplePath: string) {
  if (!confirm('Bạn có chắc muốn xoá ảnh này?')) return
  try {
    const filename = samplePath.split('/').pop()
    await http.delete(`/samples/${sampleCode}/images/${filename}`)
    fetchSamples()
  } catch (e) {
...
function openSettings() {
  Object.assign(editForm, sample.value)
  showEdit.value = true
}
...
async function saveEdit() {
  if (editForm.type === 'DEFECT' && !editForm.targetProductId) {
    alert('Vui lòng chọn sản phẩm đích cho lỗi này.')
    return
  }
  ...
  try {
    ...
    const { name, description, targetProductId } = payload
    await http.put(`/samples/${sample.value._id}`, { name, description, targetProductId })
    Object.assign(sample.value, editForm)
    closeEdit()
  } catch (e) {
    alert('Lỗi khi lưu')
  }
...
async function removeSample() {
  if (!confirm('Bạn có chắc xoá hoàn toàn mẫu vật này? Hành động này không thể hoàn tác.')) return
  submitting.value = true
  try {
    await http.delete(`/samples/${sample.value._id}`)
    router.push('/samples')
  } catch (e) {
    alert('Lỗi khi xoá')
  }
